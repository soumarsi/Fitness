...
- (void)viewDidLoad {
 ...
 NSString *C_Date=[NSString stringWithFormat:@"%@",[[image_data_array objectAtIndex:0]objectForKey:@"uploaded_date"]];
             
             
             NSDateFormatter *dateFormat = [[NSDateFormatter alloc] init];
             [dateFormat setDateFormat:@"yyyy-MM-dd"];
             NSDate *C_date = [dateFormat dateFromString:C_Date];

             NSDateFormatter *dateFormatter3 = [[NSDateFormatter alloc] init];
             [dateFormatter3 setDateFormat:@"MMMM d, YYYY"];
             NSString *convertedDateStringC = [dateFormatter3 stringFromDate:C_date];

             _current_data_lbl.text=convertedDateStringC;
    ...
    [picker dismissViewControllerAnimated:YES completion:^{
        ...
        [jsonOBJ GetJsonObjectFromURL:[NSString stringWithFormat:@"%@app_control/get_client_images?client_id=%@",App_Domain_Url,loggedin_userID] WithSpinner:nil Withblock:^(id JsonResult, NSError *error)
         
         {
             ...
             //    _goal_images.contentMode=UIViewContentModeScaleAspectFill;
